...
object Spiral2{
  ...
  def spiral(nEdges: Int, direction: Int): Element = {
    if(nEdges!=1){
      ...
      def horizontalBar = elem('-', sp.width+1, 1)
      if (direction == 0)
        horizontalBar above (sp beside space)
      else if (direction == 1)
        (space above sp) beside verticalBar
      else if (direction == 2)
        (space beside sp) above horizontalBar       
      else
        verticalBar beside (space above sp)
    }else{
      if(direction ==0 || direction == 2 ) elem("-")
      else elem("|")
    }
  }
}

println(Spiral2.spiral(20, 0))
